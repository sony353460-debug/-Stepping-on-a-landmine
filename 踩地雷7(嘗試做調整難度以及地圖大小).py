            ...
            if et[0][1]!=0 and et[0][1]!=9:                      
                for i in range(0,3):
                    for j in range(1,3):
                        ...
                        latticemap[et[0][0]-1+j][et[0][1]-1+i]=True
...
def bg():
    ...
    #在地圖上隨機取地雷的座標
    while n<mines:
        a=random.randint(0,int(bgx/50))
        b=random.randint(0,int(bgy/50))
        if a in minesx:
            if b in minesy:
                continue
            else:
...
btn1=tk.Button(cvs,text='初級',width='10',fg='green',command=bg)
btn2=tk.Button(cvs,text='中級',width='10',fg='yellow',command=bg)
btn3=tk.Button(cvs,text='高級',width='10',fg='red',command=bg)
